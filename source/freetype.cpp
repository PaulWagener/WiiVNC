...
{
	u8* buffer = (u8*)GRRLIB_TextToTexture(string, fontSize, color);
	return GX_Texture(640, fontSize*2, GX_TF_RGBA8, buffer);
}
...
extern void* GRRLIB_TextToTexture(const char *string, unsigned int fontSize, unsigned int fontColour) {
	...
	BitmapTo4x4RGBA((const unsigned char *)tempTextureBuffer,texture, 640, (fontSize*2));
