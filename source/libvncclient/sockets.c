...
#undef DEBUG_READ_EXACT
  ...
  if (n <= RFB_BUF_SIZE) {
    ...
    while (client->buffered < n) {
      int i = net_read(client->sock, client->buf + client->buffered, MIN(MAXREAD, RFB_BUF_SIZE - client->buffered));
      if (i <= 0) {
	if (i < 0) {
	  errno = -i;
#ifdef WIN32
    ...
    while (n > 0) {
      int i = net_read(client->sock, out, MIN(MAXREAD,n));
      if (i <= 0) {
	if (i < 0) {
#ifdef WIN32
	  errno=WSAGetLastError();
...
int WaitForMessage(rfbClient* client,unsigned int usecs)
{
  ...
  struct pollsd pfd[1];
  pfd[0].socket = client->sock;
  pfd[0].events = POLLIN;
  num = net_poll(pfd, 1, usecs);
  //num=net_select(client->sock+1, &fds, NULL, NULL, &timeout);
  if(num<0)
